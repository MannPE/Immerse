...
chrome.storage.sync.get(["imrdomains"], function(result) {
  ...
  if (
    result["imrdomains"] === null ||
    (Object.keys(result["imrdomains"]).length === 0 &&
...
chrome.storage.local.get(["imr-active-language"], function(lang) {
  console.log("THE LANGUAGE WE LOAD WORDS FROM IS:", lang);
  chrome.storage.local.get([lang], function(words) {
    console.log("Loaded words from imrkorean:", words);
    if (Object.keys(words).length === 0 && result.constructor === Object) {
      var wordList = {
        "thank you": {
          value: "thank you",
          translation: "감사합니다 ",
